...
export default function DropboxCallback() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        const response = await integrationsService.handleDropboxCallback(code, state);
        setStatus('success');
        setMessage(
          response.email
            ? `Conta ${response.email} conectada com sucesso!`
            : 'Conta Dropbox conectada com sucesso!',
        );
        toast.success('Conexão com o Dropbox concluída!');
        setTimeout(() => navigate('/settings/integrations'), 2000);
      } catch (err) {
        setStatus('error');
        const errorMessage =
          (err as { response?: { data?: { error?: { message?: string } } } })?.response?.data?.error?.message ||
          'Erro ao concluir a autorização com o Dropbox.';
        ...
        <div className="bg-background/80 backdrop-blur-sm border rounded-lg p-6 shadow-lg text-center space-y-6 py-8">
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="space-y-2">
              ...
              >
                {status === 'processing' && 'Conectando com o Dropbox...'}
