...
import { MCPTool, SupabaseConfig } from '@/types/integrations';
...
const SupabaseConfigDialog = ({
  ...
  const loadConfiguration = async () => {
    setIsLoading(true);
    try {
      const loadedConfig = await SupabaseService.getConfiguration(agentId);
      if (loadedConfig) {
        setConfig({
          provider: 'supabase',
  ...
  const handleConnectSupabase = async () => {
    ...
    try {
      const response = await SupabaseService.generateAuthorization(agentId);
  ...
  const handleSave = async () => {
    try {
      ...
      await SupabaseService.saveConfiguration(agentId, updatedConfig);
  ...
  const handleDisconnect = async () => {
    try {
      await SupabaseService.disconnect(agentId);
      if (onDisconnect) {
        onDisconnect();
      }
            ...
            <div className="text-center space-y-4">
              <div className="flex justify-center">
                <div className="p-4 bg-primary/10 rounded-full">
                  <BrandIcon id="supabase" size={48} className="h-12 w-12" />
                </div>
              </div>
              <div>
...
export default SupabaseConfigDialog;
