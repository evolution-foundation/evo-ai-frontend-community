...
export interface WhatsappWahaPayload {
  ...
  channel: {
    ...
    provider_config?: {
      ...
      api_key?: string;
      session_name?: string;
      // Per-channel webhook signing secret returned by
      // WahaService.verifyConnection (see WahaAuthorizationResponse) and
      // registered by the backend as this WAHA session's webhook HMAC key.
      // Carried through here so Webhooks::WhatsappController can verify
      // inbound webhooks against the persisted Channel::Whatsapp.
...
export interface WahaAuthorizationResponse {
  id: string;
  session_name: string;
  // Per-channel webhook signing secret generated by the backend at session
  // creation time. Must be threaded into the channel's provider_config on the
  // subsequent InboxesService.createChannel call so the backend can verify
  // inbound WAHA webhooks for this channel.
  webhook_hmac_key?: string;
}
